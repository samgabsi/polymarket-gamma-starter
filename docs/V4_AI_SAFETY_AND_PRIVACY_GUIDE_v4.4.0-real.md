# AI Safety And Privacy Guide - v4.4.0-real
...
The v4.4 multi-provider AI layer supports a disabled-by-default local LLM path for Ollama, llama.cpp server, LM Studio, and generic OpenAI-compatible localhost endpoints. `qwen3:8b` is the recommended default for Mac mini M4 16GB; `qwen3:4b` and `gemma3:4b` are fast/privacy-friendly fallbacks; `gemma3:12b` is experimental on 16GB. Local LLMs still require redaction, prompt governance, human review, audit hashes, and no-live-mutation controls.
