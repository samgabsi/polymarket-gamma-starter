# V4 Storage Compatibility Guide — v4.2.0-real
...
v4.2 keeps the storage layer non-destructive while adding migration export safety validation and generated manual/inventory references.
...
- v4.2 platform diagnostics, plugin manifests, schema inventories, route boundary reports, migration plans, and storage maps are local runtime records.

## Migration Policy

v4.2 provides documentation, storage maps, and non-destructive planning only. It does not automatically delete user data, move files, rewrite records, destructively migrate runtime stores, mutate live trading state, approve orders, submit orders, or cancel orders.
