# Local LLM Runtime Guide - v4.3.0-real

Polymarket OP Console v4.3.0-real adds a local LLM provider path for privacy-first operator review drafts. Local LLMs are **disabled by default**, **localhost-required by default**, **dry-run-only by default**, and governed by the same no-live-mutation rules as OpenAI.
