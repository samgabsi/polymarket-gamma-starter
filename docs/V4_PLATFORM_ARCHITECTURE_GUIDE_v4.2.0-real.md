# V4 Platform Architecture Guide — v4.2.0-real

## What v4.2 Stabilizes

v4.2.0-real stabilizes the operator intelligence platform after the v3 feature expansion. It adds shared version metadata, safety helpers, export helpers, route inventory, plugin manifest validation, platform diagnostics, storage compatibility notes, API schema inventory, shared response envelope helpers, route/module boundary metadata, and a non-destructive runtime migration planner.

v4.2 also adds modular router extraction for the platform route family, a low-risk v3 core UX API router, route ownership registry, API contract tests, generated manual source, and generated route/API/migration inventory artifacts.

## What v4.2 Does Not Change
...
Plugin manifests are metadata only. They describe future extension capabilities, allowed namespaces, forbidden capabilities, safety class, and no-live-mutation flags. v4.2 does not execute plugin code, load remote code, or grant secret access.
...
Runtime stores are created lazily under `data/` and excluded from release ZIPs. v4.2 documents compatibility for v3.5 dataset stores, v3.6 freshness stores, v3.7 task stores, v3.8 workspace stores, v3.9 cockpit stores, and v4.2 platform stores.
