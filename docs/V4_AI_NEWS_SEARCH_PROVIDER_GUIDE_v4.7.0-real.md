# AI News Search Provider Guide - v4.7.0-real
...
OpenAI web search is blocked unless `AI_NEWS_ODDS_WEB_SEARCH_ENABLED`, `OPENAI_ENABLE_API`, `OPENAI_ENABLE_RESPONSES_API`, and `OPENAI_ENABLE_WEB_SEARCH` are enabled, dry-run is disabled, operator approval/confirmation are supplied, and a key is configured. Manual evidence mode is available when web search is unavailable.
